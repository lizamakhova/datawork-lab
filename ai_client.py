...
3. ПОВТОРНОЕ ХАМСТВО ИЛИ АГРЕССИЯ:
...
"""
    ...
    def _get_smart_fallback(self, character, user_message):
        ...
        if character == "alice":
            ...
            # Слова указывающие на личное оскорбление
            personal_rude_words = ["ты дура", "ты тупая", "алиса дура", "глупые советы", "ты ничего не понимаешь", "бесполезная"]
            ...
            if any(word in message_lower for word in system_rude_words):
                ...
                # Хамят лично - устанавливаем границы
                boundary_responses = [
                    "Я тут чтобы помочь. Давай обсудим это в спокойном тоне.",
                    "Предлагаю выдохнуть и вернуться к диалогу, когда эмоции пройдут."
                ]
